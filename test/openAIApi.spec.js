...
describe('communicateWithOpenAI', () => {
  ...
  test('communicateWithOpenAI - Respuesta ok', async () => {
    ...
    const description = "El niño que vivió, famoso por derrotar a Lord Voldemort.";
    const apiKey = 'valid-api-key';

    fetch.mockResolvedValueOnce({
      ok: true,
      json: () => Promise.resolve({ choices: [{ message: { content: 'Hola ¿en qué puedo ayudarte?' } }] })
    });

    try {
      const response = await communicateWithOpenAI(name, description, prompt, apiKey);
      const data = await response.json();
      expect(data.choices[0].message.content).toBe('Hola ¿en qué puedo ayudarte?');
    } catch (error) {
